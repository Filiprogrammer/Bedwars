...
public class ColoredWoolItemShopEntry extends ItemShopEntry {

	private final int WOOL_MAX_STACK_SIZE = 64;
	
	private final int woolAmount;
	
	public ColoredWoolItemShopEntry(Material priceMaterial, int priceCount, int woolAmount) {
		super(priceMaterial, priceCount, new ItemStack(Material.WHITE_WOOL, woolAmount));
	...
	public void buy(GamePlayer gamePlayer, boolean fullStack) {
		...
		if (fullStack) {
			...
			if (maxBuyAmount > 0) {
				if (maxBuyAmount < WOOL_MAX_STACK_SIZE) {
					...
					itemAmount = woolAmount * (getPriceCountFullStack() / priceCount);
				}
